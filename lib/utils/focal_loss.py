...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha: float, 
                ...
                weight_ce: Optional[torch.Tensor] = None) -> None:
    ...
    def forward(  # type: ignore
        ...
        if self.weight_ce is not None:
            focal = focal * self.weight_ce
...
def focal_loss(
        ...
        gamma: Optional[float] = 2.0,
        reduction: Optional[str] = 'none',
        weight_ce: Optional[torch.Tensor] = None) -> torch.Tensor:
    r"""Function that computes Focal loss.

    See :class:`~kornia.losses.FocalLoss` for details.
    """
    return FocalLoss(alpha, gamma, reduction, weight_ce=weight_ce)(input, target)
