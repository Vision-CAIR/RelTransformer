...
import copy
from .image_encoder_mem_intention import MemoryAugmentedEncoder
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v):
        ...
        return self.w, torch.matmul(w, v)
...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_features,  mask_encoder):
        ...
        w, self_attention = self.attn(self.ln_1(x))
        a = x + self_attention
...
class MultiHeadModel(nn.Module):
    ...
    def forward(self, sub_label, obj_label, sub_visual, obj_visual, label_visual, encoder_features, encoder_mask):
        ...
        if sub_label is None:
            ...
            for block in self.h:
                w, hidden_states = block(hidden_states, encoder_features, encoder_mask)
                w_all.append(w)
            ...
            for block in self.h:    
                w, hidden_states = block(hidden_states)
                w_all.append(w)
            hidden_states = self.linear_projection(hidden_states)
            ...
            return w_all,  hidden_states[:, 2, :], hidden_states[:, 3, :], hidden_states[:, 4, :]
...
class reldn_head(nn.Module):
    ...
    def forward(self, spo_feat, sbj_labels=None, obj_labels=None, sbj_feat=None, obj_feat=None,all_unique_features = None):
        ...
        enc_w_all, enc_output, mask_enc = self.image_encoder(all_unique_features)
        ...
        w_all, sbj_vis_embeddings, obj_vis_embeddings, prd_hidden = self.multi_head_attention(None, None,
                                                                                       sbj_vis_embeddings,
                                                                                       obj_vis_embeddings, prd_hidden, enc_output, mask_enc)

        w_all.extend(enc_w_all)
        ...
        return prd_cls_scores, sbj_cls_scores, obj_cls_scores, w_all
