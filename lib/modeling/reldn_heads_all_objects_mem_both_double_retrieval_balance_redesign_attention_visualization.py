...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        self_att_weight,a = self._attn(query, key, value)
        ...
        a = self.c_proj(a)
        return a , alpha , self_att_weight
...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_features,  mask_encoder):
        ...
        self_attention,  memory_alpha, local_att_weight = self.attn(self.ln_1(x))
        a = x + self_attention
        ...
        att_all =dict()
        att_all["local_att"] = local_att_weight.data.cpu().numpy()
        att_all["memory_alpha"] = memory_alpha.data.cpu().numpy()
        # att_all["scene_enc_1"] = att_enc_dec1
        # att_all["scene_enc_2"] = att_enc_dec2
        att_all["enc_dec_alpha1"] = alpha1.data.cpu().numpy()
        att_all["enc_dec_alpha2"] = alpha2.data.cpu().numpy()



        return output, att_all
...
class MultiHeadModel(nn.Module):
    ...
    def forward(self, sub_label, obj_label, sub_visual, obj_visual, label_visual, encoder_features, encoder_mask):
        if sub_label is None:
            ...
            for block in self.h:
                hidden_states, att_all = block(hidden_states, encoder_features, encoder_mask)
            ...
            return hidden_states[:, 0, :], hidden_states[:, 1, :], hidden_states[:, 2, :], att_all
            ...
            for block in self.h:    
                hidden_states, att_all = block(hidden_states)
            ...
            return hidden_states[:, 2, :], hidden_states[:, 3, :], hidden_states[:, 4, :],att_all
...
class reldn_head(nn.Module):
    ...
    def forward(self, spo_feat, sbj_labels=None, obj_labels=None, sbj_feat=None, obj_feat=None,all_unique_features = None):
        ...
        sbj_vis_embeddings, obj_vis_embeddings, prd_hidden , att_all= self.multi_head_attention(None, None,
                                                                                       sbj_vis_embeddings,
        ...
        return prd_cls_scores, sbj_cls_scores, obj_cls_scores, att_all
