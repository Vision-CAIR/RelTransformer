...
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, m):
        ...
        # initialize the memory key and memory value
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        m_k = self.m_k.expand(b_s, self.m, self.h * self.d_k)
        m_v = self.m_v.expand(b_s, self.m, self.h * self.d_v)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if self.identity_map_reordering:
            ...
            att, out = self.attention(q_norm, k_norm, v_norm, attention_mask, attention_weights)
            out = queries + self.dropout(torch.relu(out))
        else:
            att, out = self.attention(queries, keys, values, attention_mask, attention_weights)
