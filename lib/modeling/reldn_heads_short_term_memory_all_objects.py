...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v,counter_fact):
        w = torch.matmul(q, k)

        if counter_fact:
            w = torch.ones(w.shape)/w.shape[-1]
        else:
    ...
    def forward(self, x, counter_fact = False):
        ...
        a = self._attn(query, key, value, counter_fact)
...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_features,  mask_encoder,counter_fact=False):
...
class MultiHeadModel(nn.Module):
    ...
    def forward(self, sub_label, obj_label, sub_visual, obj_visual, label_visual, encoder_features, encoder_mask, counter_fact=False):
        if sub_label is None:
            ...
            for block in self.h:
                hidden_states = block(hidden_states, encoder_features, encoder_mask,counter_fact)
...
class reldn_head(nn.Module):
    ...
    def forward(self, spo_feat, sbj_labels=None, obj_labels=None, sbj_feat=None, obj_feat=None,all_unique_features = None):
        ...
        sbj_vis_embeddings, obj_vis_embeddings, prd_hidden = self.multi_head_attention(None, None,
                                                                                       sbj_vis_embeddings,
                                                                                       obj_vis_embeddings, prd_hidden, enc_output, mask_enc,counter_fact=False)


        # sbj_vis_embeddings_counter_fact, obj_vis_embeddings_counter_fact, prd_hidden_counter_fact = self.multi_head_attention(None, None,
        #                                                                                     sbj_vis_embeddings,
        #                                                                                     obj_vis_embeddings, intervation_predicate, enc_output, mask_enc,
        #                                                                                     counter_fact=False)
        ...
        # print(prd_vis_embeddings.shape,prd_sem_embeddings.shape,"first")
