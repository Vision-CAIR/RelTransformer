...
import modeling.reldn_heads_all_objects_mem_both as reldn_heads
#import modeling.reldn_heads_attention1 as reldn_heads
#import modeling.reldn_heads_attention2 as reldn_heads
    ...
    def _forward(self, data, im_info, dataset_name=None, roidb=None, use_gt_labels=False, include_feat=False,  **rpn_kwargs):
        ...
        if self.training:
            if cfg.TRAIN.USE_GT_BOXES:
                ...
                rel_ret_ground_truth['sbj_rois'] = sbj_rois
                rel_ret_ground_truth['obj_rois'] = obj_rois
                rel_ret_ground_truth['rel_rois'] = rel_rois

                sbj_labels = roidb[0]['sbj_gt_classes']  # start from 0
                obj_labels = roidb[0]['obj_gt_classes']  # start from 0
                sbj_scores = np.ones_like(sbj_labels, dtype=np.float32)
                obj_scores = np.ones_like(obj_labels, dtype=np.float32)



                rel_ret_ground_truth['sbj_scores'] = sbj_scores.astype(np.float32, copy=False)
                rel_ret_ground_truth['obj_scores'] = obj_scores.astype(np.float32, copy=False)
                rel_ret_ground_truth['sbj_labels'] = sbj_labels.astype(np.int32, copy=False) + 1  
                rel_ret_ground_truth['obj_labels'] = obj_labels.astype(np.int32, copy=False) + 1  
                rel_ret_ground_truth['all_sbj_labels_int32'] = sbj_labels.astype(np.int32, copy=False)
                rel_ret_ground_truth['all_obj_labels_int32'] = obj_labels.astype(np.int32, copy=False)

                sbj_ground_truth_feat = self.Box_Head(blob_conv, rel_ret_ground_truth, rois_name='sbj_rois', use_relu=use_relu)
                obj_ground_truth_feat = self.Box_Head(blob_conv, rel_ret_ground_truth, rois_name='obj_rois', use_relu=use_relu)

                rel_ground_truth_feature = self.Prd_RCNN.Box_Head(blob_conv_prd, rel_ret_ground_truth, rois_name='rel_rois', use_relu=use_relu)
                ground_truth_concat_feat = torch.cat((sbj_ground_truth_feat, rel_ground_truth_feature, obj_ground_truth_feat), dim=1)
        ...
        # print(sbj_labels.shape, "sbj labels shape")
        # print(obj_labels.shape, "obj_labels.shape")
        # print(ground_truth_concat_feat.shape, "relation feature shape")
        # print(sbj_feat.shape, "sbj feature shape")
        # print(obj_feat.shape,"obj feature shape")
        # relation_labels =rel_ret_grou
        ...
        #     print(concat_feat.shape, group,gn_size)
        #     print(sbj_labels.shape, obj_labels.shape,sbj_feat.shape)
        ...
        if self.training:
            # print(ground_truth_concat_feat.shape,"preivous shape")
            # all_features = ground_truth_concat_feat.detach().cpu().numpy()  # shape    x* 4096 
            
            # all_unique_objects = np.unique(all_features, axis=0)

            # all_unique_objects = torch.from_numpy(all_unique_objects).cuda(sbj_feat.get_device())  # shape 2x * 4096

            # padding to be 100 * 4096


            # print(ground_truth_concat_feat.shape, "all_unique_objects shape")

            if ground_truth_concat_feat.shape[0]<100:
                #padding
                target_tensor = torch.zeros(100,4096*3).cuda(sbj_feat.get_device())
                target_tensor[:int(ground_truth_concat_feat.shape[0]), :] = ground_truth_concat_feat[:, :]
                all_unique_objects = target_tensor
            else:
                all_unique_objects = ground_truth_concat_feat[:100,:]

            all_unique_objects = all_unique_objects.unsqueeze(0)
            

            prd_cls_scores, sbj_cls_scores, obj_cls_scores , w_all= \
                    self.RelDN(concat_feat, sbj_labels, obj_labels, sbj_feat, obj_feat,all_unique_objects)
        else:
            # all_features =  concat_feat.detach().cpu().numpy()
            # all_unique_objects = np.unique(all_features, axis=0)
            # all_unique_objects = torch.from_numpy(all_unique_objects).cuda(sbj_feat.get_device())



            if concat_feat.shape[0]<100:
                #padding
                target_tensor = torch.zeros(100,4096*3).cuda(sbj_feat.get_device())
                target_tensor[:int(concat_feat.shape[0]), :] = concat_feat[:, :]
                all_unique_objects = target_tensor
            else:
        ...
        if self.training:
            ...
            entropy_loss = []
            for w in w_all:
                entropy = Categorical(probs = w).entropy()
                entropy_loss.append(entropy.mean())
            final_entropy_loss = sum(entropy_loss)/len(entropy_loss)

            return_dict['losses']['kl_entropy'] = final_entropy_loss 
