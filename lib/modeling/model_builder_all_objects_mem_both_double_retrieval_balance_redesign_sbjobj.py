...
import modeling.reldn_heads_all_objects_mem_both_double_retrieval_balance_redesign_sbj_obj as reldn_heads
    ...
    def _forward(self, data, im_info, dataset_name=None, roidb=None, use_gt_labels=False, include_feat=False,  **rpn_kwargs):
        ...
        if self.training:
            if cfg.TRAIN.USE_GT_BOXES:
                ...
                ground_truth_concat_feat = torch.cat((sbj_ground_truth_feat, rel_ground_truth_feature, obj_ground_truth_feat), dim=0)
        ...
        test_concat_feat = torch.cat((sbj_feat, rel_feat, obj_feat), dim=0)
        ...
        if self.training:
            ...
            if ground_truth_concat_feat.shape[0]<100:
                #padding
                target_tensor = torch.zeros(100,4096).cuda(sbj_feat.get_device())
            ...
            if test_concat_feat.shape[0]<100:
                #padding
                target_tensor = torch.zeros(100,4096).cuda(sbj_feat.get_device())
                target_tensor[:int(test_concat_feat.shape[0]), :] = test_concat_feat[:, :]
                all_unique_objects = target_tensor
            else:
                all_unique_objects = test_concat_feat[:100,:]
