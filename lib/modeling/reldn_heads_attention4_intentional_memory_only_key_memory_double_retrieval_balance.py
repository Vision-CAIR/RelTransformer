...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        m_v = self.m_k.expand(b_s, self.m, self.split_size)
        ...
        m_update_key = self.split_heads(m_update, k=True)
        m_update_value = self.split_heads(m_update)
        ...
        w, a1 = self._attn(a_key, m_update_key, m_update_value)
...
class MultiHeadModel(nn.Module):
    ...
    def forward(self, sub_label, obj_label, sub_visual, obj_visual, label_visual):
        ...
        if sub_label is None:
            ...
            return w_all,hidden_states[:, 0, :], hidden_states[:, 1, :], hidden_states[:, 2, :]
