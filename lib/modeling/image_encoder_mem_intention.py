...
from .mem_attention_intention import MultiHeadAttention
import math
...
class MultiLevelEncoder(nn.Module):
    ...
    def forward(self, input, attention_weights=None):
        ...
        for l in self.layers:
            w, out = l(out, out, out, attention_mask, attention_weights)
            outs.append(out.unsqueeze(1))
            w_all.append(w)
        outs = torch.cat(outs, 1)
        return w_all, outs, attention_mask
...
class MemoryAugmentedEncoder(MultiLevelEncoder):
    def __init__(self, N, padding_idx, d_in=4096*3, **kwargs):  
