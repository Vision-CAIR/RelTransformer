...
class reldn_head(nn.Module):
    ...
    def forward(self, spo_feat, sbj_labels=None, obj_labels=None, sbj_feat=None, obj_feat=None):
        ...
        intervation_predicate = torch.zeros(prd_hidden.shape).cuda(device_id)
        intervation_sbj_vis = torch.zeros(sbj_vis_embeddings.shape).cuda(device_id)
        intervation_obj_vis = torch.zeros(obj_vis_embeddings.shape).cuda(device_id)

        sbj_vis_embeddings, obj_vis_embeddings, prd_hidden = self.multi_head_attention(sbj_vecs, obj_vecs,
                                                                                       sbj_vis_embeddings,
                                                                                       obj_vis_embeddings, prd_hidden)
        sbj_vis_embeddings_zombie,  obj_vis_embeddings_zombie, predicate_counterfact = self.multi_head_attention(sbj_vecs,  obj_vecs,
                                                                                        sbj_vis_embeddings, obj_vis_embeddings,
                                                                                        intervation_predicate)


        sbj_visual_counterfact, obj_vis_embeddings_zombie, predicate_zombie = self.multi_head_attention(sbj_vecs,  obj_vecs, intervation_sbj_vis, obj_vis_embeddings, prd_hidden)

        
        sbj_visual_zombie, obj_visual_counterfact, predicate_zombie = self.multi_head_attention(sbj_vecs, obj_vecs, sbj_vis_embeddings, intervation_obj_vis, prd_hidden)


        sbj_vis_embeddings = sbj_vis_embeddings - sbj_visual_counterfact

        obj_vis_embeddings = obj_vis_embeddings - obj_visual_counterfact

        prd_hidden = prd_hidden - predicate_counterfact
        ...
        prd_vis_embeddings = self.prd_vis_embeddings(prd_hidden)
